...
namespace DTValidator {
	...
	{
		private static string[] _excludedPathsStarts =
		{
			"Packages/", 
			"Assets/Settings/"
		};
		...
		private static IEnumerable<Scene> GetSavedScenes()
		{
			...
			return guids.Select(AssetDatabase.GUIDToAssetPath)
				.Where(path => _excludedPathsStarts.None(path.StartsWith))
				.Select(EditorSceneManager.OpenScene);
		}

		private static IEnumerable<Scene> GetOpenScenes() 
		{
			var guids = EditorSceneManager.GetSceneManagerSetup().Select(scene => scene.path).ToArray();
		...
		{
			var guids = new string[SceneManager.sceneCountInBuildSettings];
			for (var i = 0; i < guids.Count(); i++) 
				guids[i] = SceneUtility.GetScenePathByBuildIndex(i);

			return guids.Select(EditorSceneManager.OpenScene);
		}
		...
		private static IEnumerable<ScriptableObject> GetSavedScriptableObjects() 
		{
			...
			return guids.Select(AssetDatabase.GUIDToAssetPath)
				.Where(path => _excludedPathsStarts.None(path.StartsWith))
				.Select(path => AssetDatabase.LoadAssetAtPath(path, typeof(ScriptableObject)) as ScriptableObject);
		}
